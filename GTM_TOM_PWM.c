...
#define LED                 IfxGtm_TOM1_8_TOUT0_P02_0_OUT          /* LED which will be driven by the PWM          */
#define Test_PIN1           IfxGtm_TOM1_11_TOUT3_P02_3_OUT
#define BASE_FREQ           (2000000.0f/256.0f)
#define PWM_HZ              (100.0f)
#define PWM_PERIOD          BASE_FREQ/PWM_HZ                                 /* PWM period for the TOM*/
#define FADE_STEP           PWM_PERIOD / 100.0f
/*********************************************************************************************************************/
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
...
void initGtmTomPwm(void)
{
    ...
    IfxGtm_Cmu_enableClocks(&MODULE_GTM, IFXGTM_CMU_CLKEN_FXCLK|IFXGTM_CMU_CLKEN_CLK0);   /* Enable the FXU clock                         */

    /* Initialize the configuration structure with default parameters */
    IfxGtm_Tom_Pwm_initConfig(&g_tomConfig, &MODULE_GTM);

    //g_tomConfig.tom = LED.tom;                                      /* Select the TOM depending on the LED          */
    g_tomConfig.tom = Test_PIN1.tom;                           /* Select the channel depending on the LED      */
    //g_tomConfig.tomChannel = LED.channel;                           /* Select the channel depending on the LED      */
    g_tomConfig.tomChannel = Test_PIN1.channel;                           /* Select the channel depending on the LED      */
    g_tomConfig.period = PWM_PERIOD;                                /* Set the timer period                         */
    //g_tomConfig.pin.outputPin = &LED;                               /* Set the LED port pin as output               */
    g_tomConfig.pin.outputPin = &Test_PIN1;                               /* Set the LED port pin as output               */
